...
var page = {
	...
	init: function() {
		...
		idPalestra = window.location.pathname.match(/participantes\/?\/([0-9]+)/); //? para atividade ou atividades
		...
		this.collectionView.on('rendered',function(){
			...
			function selecionarTudo(){
				$('#selecionarTudo').parent().toggleClass('btn-success');
				
				if($('#selecionarTudo').prop('checked')){
					$('#selecionarTudo').parent().find('span').text('Desmarcar todos os participantes');
					$(this).parent().find('span').text('Desmarcar todos os participantes');
					$('input[type=checkbox]').prop('checked',true);
				} else {
					$('#selecionarTudo').parent().find('span').text('Marcar todos os participantes');
					$('input[type=checkbox]').prop('checked',false);
				}
			}
			
			$('#selecionarTudo').click(function(e) {
				selecionarTudo();
			});
			
			selecionarTudo();
			
			//ALERTA ANIMADO DA ATA
			...
			$('#filter').bind('keyup focus',function() {
				var that = this;
				var onde = $('.collection tbody tr');
				var total = 0;
				$.each(onde,
				function(i, val) {
					var results = app.parseURL($(val).text().toLowerCase()).indexOf(app.parseURL($(that).val().toLowerCase()));
				...
				if(total == 0)
					$('#nenhumResultado').removeClass('hide');
				else
					$('#nenhumResultado').addClass('hide');
			});
			
			
			
			//PRESENÇA DO PARTICIPANTE NA PALESTRA	
			...
			$('table.collection tbody tr').click(function(e) {
				...
				var presencaBackbone = new model.PalestraParticipanteCollection();
					...
					presencaBackbone.fetch({

						data: { idParticipante: this.id, idPalestra: idPalestra[1] },

						success: function(todos) {
							
							
							var atual = todos.first();
							
							//CHECKBOX
							var cbx = $('#cbPresencaParticipante'+atual.get('idParticipante'));
							var presenca = 0;
							...
							console.log(atual.get('idPalestra'));
					
							atual.save({
								//MUDA ISSO
								'presenca': presenca,
								//MANTEM ISSO-->
								'idParticipante': atual.get('idParticipante'),
								'idPalestra': atual.get('idPalestra'),
								'idCertificado': atual.get('idCertificado')
							}, {
								wait: true,
			...
			//NÃO ABRE MODAL AO CLICAR NA LINHA
		...
		this.fetchParticipantes({ page: 1, orderBy: 'Nome', orderDesc: '', idPalestra: 128 });
	 ...
	 */
	fetchParticipantes: function(params, hideLoader, checked) {
		...
		page.participantes.fetch({
			...
			success: function(part) {
				...
				if (page.participantes.collectionHasChanged) {
					...
					//MARCA PRESENÇA LOGO DO BANCO DE DADOS		
							
					var participantes = $.getJSON(base+'api/palestraparticipantes?idPalestra='+idPalestra[1]);
					
					participantes.success(function(todos){
						$.each( todos.rows , function(key, atual) {
							console.log(atual.idParticipante);
							if(parseInt(atual.presenca) === 1)
								$('#cbPresencaParticipante'+atual.idParticipante).prop('checked',true);
						});
					});
