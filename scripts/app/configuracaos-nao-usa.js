...
var page = {
	...
	configuracaos: new model.ConfiguracaoCollection(),
	collectionView: null,
	configuracao: null,
	modelView: null,
	isInitialized: false,
	...
	init: function() {
		...
		$('#filter').change(function(obj) {
			...
			page.fetchConfiguracaos(page.fetchParams);
		});
		...
		this.collectionView.on('rendered',function(){
			...
			$('table.collection tbody tr').click(function(e) {
				e.preventDefault();
				var m = page.configuracaos.get(this.id);
		...
		this.fetchConfiguracaos({ page: 1 });
		...
		if (model.longPollDuration > 0)	{
			setInterval(function () {
				...
				if (!page.dialogIsOpen)	{
					page.fetchConfiguracaos(page.fetchParams,true);
				}
	 ...
	 */
	fetchConfiguracaos: function(params, hideLoader) {
		...
		page.configuracaos.fetch({

			data: params,

			success: function() {

				if (page.configuracaos.collectionHasChanged) {
...
$('#icone-acao-modal').removeClass('icon-plus-sign');
	...
	updateModel: function() {
		...
		page.configuracao.save({
			...
			'imagemLogo': $('input#imagemLogo').val(),
			'cnpj': $('input#cnpj').val(),
			...
			success: function(){
				...
				setTimeout("app.appendAlert('Configuracao foi " + (isNew ? "inserido" : "editado") + " com sucesso','alert-success',3000,'collectionAlert')",500);
				app.hideProgress('modelLoader');

				// if the collection was initally new then we need to add it to the collection now
				if (isNew) { page.configuracaos.add(page.configuracao) }
				...
				$('table.collection tr#'+page.configuracao.id).addClass('modificou-item');	
		},
