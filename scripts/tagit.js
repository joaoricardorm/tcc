...
(function ($) {
    $.widget("ui.tagit", {
        ...
        // default options
        options:{
			//foca o inputd e insercaoo ao carregar a pagina
        ...
        _create:function () {
            ...
            this.element.children('li').each(function () {
				...
				var tagClass = tag.attr('tagClass') || tag.data('class');
                self.options.initialTags.push({label:tag.text(), value:(tagValue ? tagValue : tag.text()), class:tag.tagClass});
            });
			...
			if(this.options.focusInputOnLoad)
				this.input.focus();
            ...
            $(this.element).click(function (e) {
                if ($(e.target).hasClass('tagit-close')) {
					...
					$('.tag-choice input').click(function (e) {
						console.log('passo');
        ...
        _addTag:function (label, value, classe) {
            ...
            tag.element = $('<li class="tagit-choice '+classe+'"'
            ...
            if (this.options.initialTags.length != 0) {
                $(this.options.initialTags).each(function (i, element) {
                    if (typeof (element) == "object")
                        input._addTag(element.label, element.value, element.class);
                    else
        ...
        add:function (label, value, classe) {
            if(typeof(label) == "object")
                return this._addTag(label.label, label.value, label.classe);
            else
                return this._addTag(label, value, classe);
        },
