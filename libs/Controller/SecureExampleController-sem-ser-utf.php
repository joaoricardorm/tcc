...
class SecureExampleController extends AppBaseController
{
	...
	public function Login()
	{
		...
		else
		{
				...
				require_once("Model/ConfiguracaoCriteria.php");
			
				$filtro = new ConfiguracaoCriteria();
				$filtro->IdConfiguracao_GreaterThan = 1;
				
				$configuracao = $this->Phreezer->Query('Configuracao',$filtro);
		foreach ($configuracao as $c){
				echo $c->NomeInstituicao;
		}
			// login failed
			$this->Rediressct('SecureExample.LoginForm','Unknown username/password combination');
		}
	}
